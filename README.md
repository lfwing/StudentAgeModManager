...
尝试 GitHub 直连与文本镜像，获取失败也不影响本地订阅的发现、启停和接入。
...
等待下载完成
        ↓
游戏关闭时点“同步刷新”，或下次启动游戏时接入
```

“打开工坊页面”会在 Steam 已运行时直接进入客户端中的对应工坊项目；Steam 未运行或
客户端协议调用失败时会回退到官方网页。订阅和取消订阅仍然在 Steam 中完成。

安装 Bridge 后的新订阅，通常会在下载完成后点击“同步刷新”或下一次游戏启动时自动启用。
以前已经订阅的项目属于首次基线，不会被批量擅自开启；它们会显示在管理器中，可直接点击
“启用”，也可以在游戏“本地”页开启。
...
- 管理器只允许在游戏关闭时切换，操作后会立即同步 Bridge 联接，下次启动游戏生效；
...
| `Steam 工坊 · … · 未启用` | 已订阅且能识别 DLL 包，但当前用户已关闭该项目 |
| `Steam 工坊 · … · 更新中` | Steam 尚未完成下载或作者更新，当前不会建立联接 |
| `Steam 工坊 · … · 待同步` | `_mod` 中已启用，但 Bridge 联接尚未成功建立 |
| `Steam 工坊 · … · 声明缺失 / 包无效` | 缺少或不符合 `workshop-plugin.json` 包规范，Bridge 拒绝接入 |
...
| `本地 · 未收录 · 重复 GUID` | 与另一个本地或工坊插件单元使用了相同 GUID |

颜色规则：

- “已收录 / 已接入 / 已启用”为绿色；
- “未收录 / 未接入 / 未启用”为红色；
- 更新、待同步、路径冲突和重复 GUID 为橙色。
...
可以。只要项目已订阅、下载完成、已启用，并且工坊包符合 Bridge 格式，就可以接入。

管理器从本地 Steam 订阅数据发现项目，不依赖 Git 收录或公开页面。未收录、私密项目即使
当前禁用、没有联接，只要本地包带合法声明也会继续显示；启用后状态会变为：
...
如果管理器能读取本机 Steam 状态，会进一步显示“未启用”“更新中”“待同步”“声明缺失”
或“包无效”。仍只显示“未接入”通常表示尚未订阅，或当前无法可靠读取本地 Steam 用户状态。
常见原因包括：

- Steam 还没有下载完成；
- 项目正在更新；
- 属于首次基线，尚未在管理器或游戏“本地”页开启；
- 玩家已经手动关闭，状态应显示“未启用”；
- 工坊包缺少合法声明；
- Bridge 尚未随游戏启动运行，也没有在游戏关闭时点击“同步刷新”。
...
没有进入推荐目录、但带合法 DLL 声明的本地订阅仍会显示在前面的“Steam 创意工坊目录”中，
不会因为在线索引暂时不可用而被归入“本地已安装插件”。

### 在管理器或游戏“本地”页关闭后还会自动开启吗？

不会。Bridge 会尊重手动关闭。禁用只会从当前用户的 `_mod` 中移除 ID 并撤销 Bridge 联接，
不会取消订阅或改动工坊源目录。Steam 仍会照常下载作者更新，但项目保持禁用；以后点“启用”
会直接联接更新后的最新版。取消后重新订阅同一个 Workshop ID，也不会强制再次开启。
...
每个 Steam 用户都有独立的首次基线。第一次同步时，已有订阅只会被登记，不会全部自动开启；
之后的新合法订阅会在下载完成后点击“同步刷新”或下一次启动游戏时自动开启。
